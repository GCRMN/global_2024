...
The `r region_i` GCRMN region ranges over a longitudinal axis of `r axis_long` km and a latitudinal axis of `r axis_lat` km. The region hosts `r extent_abs` km^2^ of coral reefs, which represent `r extent_rel`% of the world’s coral reef extent. `r region_i` is the `r extent_rank` largest GCRMN region in terms of coral reef extent. A total of `r nb_countries` `r denomination` host coral reefs of the region. Coral reefs range from `r border_north` in the North to `r border_south` in the South, and from `r border_west` in the West to `r border_east` in the East (**Fig. 2.`r nb_chapter`.1**).
