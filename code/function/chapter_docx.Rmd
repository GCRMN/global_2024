...
From 2020, a total of `r pop_2020` inhabitants were living within 5 km of coral reefs of the `r region_i` region. From 2000 to 2020, the region recorded an increase of `r pop_change_rel`% in the populations residing within 5 km of coral reefs, representing an average annual increase of `r pop_change_abs` inhabitants. The greatest increase in populations over this period was recorded in sub-region `r pop_change_abs_max`, although the highest number of inhabitants in 2020 is located in sub-region `r pop_2020_max` (**Supplementary Table X**).
...
**Figure 2.`r nb_chapter`.6.** Percentage of the coral reefs within the `r region_i` region experiencing heat stress levels per year, from 1986 to 2024. The level “No stress” corresponds to values of Degree Heating Weeks (DHW) equal to 0, the level “Warning” to 0 < DHW < 4 (risk of possible bleaching), the level “Alert 1” to 4 <= DHW < 8 (risk of reef-wide bleaching), the level “Alert 2” to 8 <= DHW < 12 (risk of reef-wide bleaching with mortality of heat-sensitive corals), the level “Alert 3” to 12 <= DHW < 16 (risk of multi-species mortality), the level “Alert 4” to 16 <= DHW < 20 (risk of severe, multi-species mortality), the level “Alert 5” to 20 <= DHW (risk of near complete mortality).
...
`r first_sentence`From 1980 to 2025, a total of `r data_cyclones_nb` `r regional_cyclone_name` have passed within 100 km of coral reefs within the `r region_i` region, representing an average of `r data_cyclones_mean` ± `r data_cyclones_sd` SD `r regional_cyclone_name` per year (**Fig. 2.`r nb_chapter`.7**). The `r gsub('.{1}$', '', regional_cyclone_name)` with the highest sustained wind speed over the studied period was the `r gsub('.{1}$', '', regional_cyclone_name)` `r data_cyclones_highest_name`, in `r data_cyclones_highest_date`, which passed `r data_cyclones_highest_dist` km from a coral reef with sustained wind speed of `r data_cyclones_highest_speed` km.h^-1^ (**Fig. 2.`r nb_chapter`.8**). Although we selected the 100 km threshold, `r regional_cyclone_name` passing at a greater distance can have an impact coral reefs [1].
...
The estimation of temporal trends in percentage cover of benthic categories from 1980 to 2024 was assessed using data from `r nb_datasets` datasets, representing `r nb_sites` monitoring sites, which were acquired through `r nb_surveys` surveys conducted between `r first_year` to `r last_year`. On all of these surveys, 90% were assessed after `r surveys_90_perc` (**Supplementary Figure 1**). Although `r max_surveys_value`% of all surveys (`r max_sites_value`% of sites) conducted in the region between 1980 and 2024 was carried out in sub-region `r max_surveys_name`, only `r min_surveys_value`% was carried out in sub-region `r min_surveys_name` (`r min_sites_value`% of sites).
